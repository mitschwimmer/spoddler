...
public class PlayerSession implements Closeable, PlayerQueueEntry.Listener {
    ...
    private void add(@NotNull PlayableId playable, boolean preloaded) {
        ...
        if (queue.next() == entry) {
            ...
            if (head != null && head.crossfade != null) {
                ...
                if ((fadeOut = head.crossfade.selectFadeOut(PlaybackMetrics.Reason.TRACK_DONE, customFade)) != null)
    ...
    public void instantReached(@NotNull PlayerQueueEntry entry, int callbackId, int exactTime) {
        switch (callbackId) {
            ...
            case PlayerQueueEntry.INSTANT_START_NEXT:
                executorService.execute(() -> advance(PlaybackMetrics.Reason.TRACK_DONE));
                break;
        ...
        if (entry == queue.head()) {
            if (ex instanceof PlayableContentFeeder.ContentRestrictedException) {
                ...
                executorService.execute(() -> {
                    ...
                    playInternal(newEntry.playable, lastPlayPos, lastPlayReason == null ? PlaybackMetrics.Reason.TRACK_ERROR : lastPlayReason);
                });
                return;
            }
     ...
     */
    @Contract("_, _, _ -> new")
    private @NotNull EntryWithPos playInternal(@NotNull PlayableId playable, int pos, @NotNull PlaybackMetrics.Reason reason) {
        lastPlayPos = pos;
     ...
     */
    @NotNull
    public String play(@NotNull PlayableId playable, int pos, @NotNull PlaybackMetrics.Reason reason) {
    ...
    public interface Listener {
        ...
        /**
         * The current track changed. Not called if PlayerSession#playInternal(PlayableId, int, Reason) is called directly.
         *
         ...
         */
        void trackChanged(@NotNull String playbackId, @Nullable TrackOrEpisode metadata, int pos, @NotNull PlaybackMetrics.Reason startedReason);
         ...
         */
        void trackPlayed(@NotNull String playbackId, @NotNull PlaybackMetrics.Reason endReason, @NotNull PlayerMetrics playerMetrics, int endedAt);
    }
