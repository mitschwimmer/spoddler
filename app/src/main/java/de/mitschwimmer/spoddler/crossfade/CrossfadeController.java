...
public class CrossfadeController {
    ...
    private void populateFadeIn(@NotNull Map<String, String> metadata) {
        ...
        if (fadeInDuration != 0 && fadeInCurves.size() > 0)
            fadeInMap.put(PlaybackMetrics.Reason.TRACK_DONE, new FadeInterval(fadeInStartTime, fadeInDuration, LookupInterpolator.fromJson(getFadeCurve(fadeInCurves))));
        else if (defaultFadeDuration > 0)
            fadeInMap.put(PlaybackMetrics.Reason.TRACK_DONE, new FadeInterval(0, defaultFadeDuration, new LinearIncreasingInterpolator()));
        ...
        if (fwdFadeInDuration > 0)
            fadeInMap.put(PlaybackMetrics.Reason.FORWARD_BTN, new FadeInterval(fwdFadeInStartTime, fwdFadeInDuration, new LinearIncreasingInterpolator()));
        ...
        if (backFadeInDuration > 0)
            fadeInMap.put(PlaybackMetrics.Reason.BACK_BTN, new FadeInterval(backFadeInStartTime, backFadeInDuration, new LinearIncreasingInterpolator()));
    }
    ...
    private void populateFadeOut(@NotNull Map<String, String> metadata) {
        ...
        if (fadeOutDuration != 0 && fadeOutCurves.size() > 0)
            fadeOutMap.put(PlaybackMetrics.Reason.TRACK_DONE, new FadeInterval(fadeOutStartTime, fadeOutDuration, LookupInterpolator.fromJson(getFadeCurve(fadeOutCurves))));
        else if (defaultFadeDuration > 0)
            fadeOutMap.put(PlaybackMetrics.Reason.TRACK_DONE, new FadeInterval(trackDuration - defaultFadeDuration, defaultFadeDuration, new LinearDecreasingInterpolator()));


        int backFadeOutDuration = Integer.parseInt(metadata.getOrDefault("audio.backbtn.fade_out_duration", "-1"));
        if (backFadeOutDuration > 0)
            fadeOutMap.put(PlaybackMetrics.Reason.BACK_BTN, new PartialFadeInterval(backFadeOutDuration, new LinearDecreasingInterpolator()));

        int fwdFadeOutDuration = Integer.parseInt(metadata.getOrDefault("audio.fwdbtn.fade_out_duration", "-1"));
        if (fwdFadeOutDuration > 0)
            fadeOutMap.put(PlaybackMetrics.Reason.FORWARD_BTN, new PartialFadeInterval(fwdFadeOutDuration, new LinearDecreasingInterpolator()));
    }
     ...
     */
    @Nullable
    public FadeInterval selectFadeIn(@NotNull PlaybackMetrics.Reason reason, boolean customFade) {
     ...
     */
    @Nullable
    public FadeInterval selectFadeOut(@NotNull PlaybackMetrics.Reason reason, boolean customFade) {
        if ((!customFade && fadeOutPlayable != null) && reason == PlaybackMetrics.Reason.TRACK_DONE) {
            fadeOut = null;
            activeInterval = null;
